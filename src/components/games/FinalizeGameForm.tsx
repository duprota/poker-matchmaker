      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {players.map(player => <div key={player.id} className="flex items-center gap-4">
                <span className="min-w-[150px]">{player.player.name}</span>
                <input type="number" inputMode="numeric" pattern="[0-9]*" value={results[player.id] || ""} onChange={e => handleResultChange(player.id, e.target.value)} style={{
              appearance: 'textfield',
              MozAppearance: 'textfield',
              WebkitAppearance: 'none'
            }} className="max-w-[120px] border border-input px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-md bg-transparent" />
              </div>)}
          </div>

          {difference !== 0 && <Alert variant="destructive">
              <AlertDescription>
          ...
          <div className="flex justify-between pt-4">
            ...
            <Button onClick={handleFinalize} disabled={difference !== 0 || updating}>
